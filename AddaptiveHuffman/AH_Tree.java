...
public class AH_Tree
{
    ...
    private Node swap_by(Node temp)
    {
        ...
        while(!nodes.isEmpty())
        {
            ...
            if(next != temp && !IsParent(next,temp))
            {
                if(next.counter <= temp.counter && next.node_number > temp.node_number)
                {
                    if (swap_by == null)
                        swap_by = next;
                    else if (next.counter <= swap_by.counter && next.node_number > swap_by.node_number)
                        swap_by = next;
                }
            }
        }
